import define1 from "./7dbc780f5356575a@2710.js";
import define2 from "./e3c0dcb49fca94d3@288.js";
import define3 from "./e2cca0a7645d62fa@764.js";
import define4 from "./13e1d08a1a1ed038@3590.js";
import define5 from "./64641700df65baed@91.js";
import define6 from "./f92778131fd76559@1208.js";
import define7 from "./3d321b3f7d398726@482.js";
import define8 from "./0b16aaecf59cca0a@803.js";
import define9 from "./c4047069e47f8784@249.js";
import define10 from "./048a17a165be198d@271.js";
import define11 from "./bad810ff1e80611b@137.js";

function _1(md){return(
md`# GESI Survey | Designer Tools`
)}
...
function _survey(Inputs,surveys,localStorageView,URLSearchParams,location){return(
Inputs.bind(Inputs.select(surveys, {label: "survey"}), localStorageView("designer-project", {
  defaultValue: 
...
function _surveyUi(initialLoadQuestions,initialLoadLayout,load_config,view,pageHeader,Inputs,buttonLabel,exportUi,importUi,surveyEditor,pageFooter)
{
...
function _16(questionsNoLayout){return(
questionsNoLayout
)}
...
function _25(md,selectedQuestionDiff,questions,surveyOutput){return(
md`
...
function _autosave(saveQuestions,$0,saveLayout,saveConfig,files,$1,invalidation,settings,Generators)
{
...
function _33(htl,exportDataUri){return(
...
function _revertChanges(files,$0,$1,Event){return(
async function() {
  ...
  await files.save("settings.json", $0.value);
  $1.dispatchEvent(new Event('input', {bubbles: true})) // reload everything
}
)}
...
function _54(d3,exportQuestionsCSV){return(
d3.csvFormat(exportQuestionsCSV)
)}
...
async function _questionsCsvDataUri(d3,exportQuestionsCSV){return(
URL.createObjectURL(new Blob([ await d3.csvFormat(exportQuestionsCSV) ], { type: 'text/csv' }))
)}
...
function _downloadQuestionsCsv(htl,questionsCsvDataUriView){return(
htl.html`<a href=${questionsCsvDataUriView} download="questions_${Date.now()}.csv">
...
function _downloadLayoutCsv(htl,layoutCsvDataUriView,exportLayoutProblems,md){return(
htl.html`<a href=${layoutCsvDataUriView} download="layout_${Date.now()}.csv">
  Download layout.csv
</a>
...
function _sampleExportUi(questionsCsvDataUriView,layoutCsvDataUriView,exportUi)
{
...
function _82(sampleImportUi){return(
sampleImportUi
)}
...
function _92(questionsNoLayout,dataEditor,md)
{
...
function _93(questionsNoLayout){return(
...
function _95(layoutsNoQuestion,dataEditor,md)
{
...
All options need values defined, this is the key used to ensure updates to question text do not affect the endusers pre-existing answers.
`
)}

function _97(optionsWithoutValue,md)
{
...
function _configImportInput(Inputs){return(
Inputs.text({
...
async function _configImporter(configImportInput,$0,saveConfig,files,$1,Event)
{
...
async function* _save_config($0,_,latestConfig,md,saveConfig,files,$1)
{
...
function _responses(surveyView,surveyOutput){return(
surveyView(surveyOutput.config.style, surveyOutput.questions, surveyOutput.layout, surveyOutput.config, new Map(), {
    putFile: (name) => console.log("mock save " + name),
    getFile: (name) => console.log("mock get " + name),
  })
)}

function _121(surveyOutput){return(
surveyOutput.config
)}
...
function _123(responses){return(
responses
)}
...
function _146(substratum,invalidation){return(
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof survey")).define("viewof survey", ["Inputs","surveys","localStorageView","URLSearchParams","location"], _survey);
  ...
  main.variable(observer("viewof surveyUi")).define("viewof surveyUi", ["initialLoadQuestions","initialLoadLayout","load_config","view","pageHeader","Inputs","buttonLabel","exportUi","importUi","surveyEditor","pageFooter"], _surveyUi);
  ...
  main.variable(observer()).define(["questionsNoLayout"], _16);
  ...
  main.variable(observer()).define(["md","selectedQuestionDiff","questions","surveyOutput"], _25);
  ...
  main.variable(observer("autosave")).define("autosave", ["saveQuestions","viewof surveyOutput","saveLayout","saveConfig","files","viewof settings","invalidation","settings","Generators"], _autosave);
  main.variable(observer()).define(["md"], _31);
  main.variable(observer("exportDataUri")).define("exportDataUri", ["surveyOutput"], _exportDataUri);
  main.variable(observer()).define(["htl","exportDataUri"], _33);
  ...
  main.variable(observer("revertChanges")).define("revertChanges", ["files","viewof settings","viewof survey","Event"], _revertChanges);
  ...
  main.variable(observer()).define(["d3","exportQuestionsCSV"], _54);
  ...
  main.variable(observer("downloadQuestionsCsv")).define("downloadQuestionsCsv", ["htl","questionsCsvDataUriView"], _downloadQuestionsCsv);
  ...
  main.variable(observer("downloadLayoutCsv")).define("downloadLayoutCsv", ["htl","layoutCsvDataUriView","exportLayoutProblems","md"], _downloadLayoutCsv);
  ...
  main.variable(observer("sampleExportUi")).define("sampleExportUi", ["questionsCsvDataUriView","layoutCsvDataUriView","exportUi"], _sampleExportUi);
  ...
  main.variable(observer()).define(["sampleImportUi"], _82);
  ...
  main.variable(observer()).define(["questionsNoLayout","dataEditor","md"], _92);
  main.variable(observer()).define(["questionsNoLayout"], _93);
  main.variable(observer()).define(["md"], _94);
  main.variable(observer()).define(["layoutsNoQuestion","dataEditor","md"], _95);
  main.variable(observer()).define(["md"], _96);
  main.variable(observer()).define(["optionsWithoutValue","md"], _97);
  ...
  main.variable(observer("viewof configImportInput")).define("viewof configImportInput", ["Inputs"], _configImportInput);
  main.variable(observer("configImportInput")).define("configImportInput", ["Generators", "viewof configImportInput"], (G, _) => G.input(_));
  main.variable(observer("configImporter")).define("configImporter", ["configImportInput","viewof surveyConfig","saveConfig","files","viewof settings","Event"], _configImporter);
  main.variable(observer("save_config")).define("save_config", ["viewof surveyConfig","_","latestConfig","md","saveConfig","files","viewof settings"], _save_config);
  ...
  main.variable(observer("viewof responses")).define("viewof responses", ["surveyView","surveyOutput"], _responses);
  main.variable(observer("responses")).define("responses", ["Generators", "viewof responses"], (G, _) => G.input(_));
  main.variable(observer()).define(["surveyOutput"], _121);
  main.variable(observer()).define(["md"], _122);
  main.variable(observer()).define(["responses"], _123);
  ...
  const child3 = runtime.module(define3).derive(["REGION"], main);
  main.import("listObjects", child3);
  main.import("getObject", child3);
  main.import("putObject", child3);
  main.import("listUsers", child3);
  main.import("createUser", child3);
  main.import("deleteUser", child3);
  main.import("getUser", child3);
  main.import("listAccessKeys", child3);
  main.import("createAccessKey", child3);
  main.import("deleteAccessKey", child3);
  main.import("viewof manualCredentials", child3);
  main.import("manualCredentials", child3);
  main.import("viewof mfaCode", child3);
  main.import("mfaCode", child3);
  main.import("saveCreds", child3);
  main.import("listUserTags", child3);
  main.import("tagUser", child3);
  main.import("untagUser", child3);
  main.import("iam", child3);
  main.import("s3", child3);
  main.import("listGroups", child3);
  main.import("listGroupsForUser", child3);
  main.import("addUserToGroup", child3);
  main.import("removeUserFromGroup", child3);
  ...
  main.import("createQuestion", child4);
  main.import("reifyAttributes", child4);
  main.import("bindLogic", child4);
  main.import("setTypes", child4);
  main.import("surveyView", child4);
  main.import("config", child4);
  ...
  main.import("fileInput", child7);
  const child8 = runtime.module(define8);
  main.import("dataEditor", child8);
  const child9 = runtime.module(define9);
  main.import("pageHeader", child9);
  main.import("pageFooter", child9);
  main.import("buttonLabel", child9);
  const child10 = runtime.module(define10);
  main.import("localStorageView", child10);
  main.variable(observer()).define(["md"], _144);
  const child11 = runtime.module(define11);
  main.import("substratum", child11);
  main.variable(observer()).define(["substratum","invalidation"], _146);
  return main;
}
