...
viewof parameters = Inputs.bind(
  exporter({ style: default_style, output: (out) => (mutable output = out) }),
...
```js
TRACE_MODULE = "@tomlarkworthy/sequencer"
```
...
```js
module_map = moduleMap(task_runtime)
```
...
```js
viewof test_module_selection = Inputs.select(
  [
    undefined,
...
```js
test_notebook = import(test_module_selection)
```
...
```js
test_notebook_define = test_notebook.default.toString()
```

```js
lopecode_generated_define = generate_define(
...
```js
differences = compareDefines(lopecode_generated_define, test_notebook_define)
```

```js
test_notebook_module = {
...
```js
import { thisModule, keepalive } from "@tomlarkworthy/runtime-sdk"
```
