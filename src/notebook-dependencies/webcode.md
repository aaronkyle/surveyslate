...
## ** Rapid HTTP endpoint development featuring *production debugging*, *instant deploys*** and [Observable](https://observablehq.com/) browser based tooling.
...
Do you like the idea of Serverless functions-at-a-service but find the sheer pain of development is actually worse than the operation complexity it displaces? Do you empathize with the following:-
...
### Extending [Observable](https://observablehq.com/) to the backend.
...
```js
const deploy = (name, handler, options = {}) => {
...
```js
import { serverlessCellUI, colors } from "@endpointservices/webcode-ui"
```

```js
//import {deploy as baseDeploy, getContext, subdomain} from '@endpointservices/serverless-cells'
import {deploy as baseDeploy, getContext, subdomain} from '/components/serverless-cells.js';
display(baseDeploy);
display(getContext);
display(subdomain);
```
