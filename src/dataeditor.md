...
```js
const selectable = view(Inputs.toggle({
  label: "select?",
  value: true
}))
```
...
```js
const deletable = view(Inputs.toggle({
  label: "delete?",
  value: true
}))
```
...
You can add add/delete rows by assigning to the **data** subview. This can be useful for external input forms to add/remove elements. Note it does not raise an input event itself`
  ...
  let newItem = view`
    ...
    ${Inputs.button("Add Item", {
      reduce: () => {
        ...
        // viewof example.dispatchEvent(new Event("input", { bubbles: true }));
